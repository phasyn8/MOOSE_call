...
# ++++++++++++++++++++++++++++++ INIT ++++++++++++++++++++++++++++++++++++++
    ...
    def __init__(self, filename, file_path, mesh_path, version, parameters_df, surface_elev, **kwargs):
        ...
        #Required arguments
            ...
            volAndFaults_list = [self.vol_meshBlocks[1:-1], self.fault_meshBlocks[1:-1]]
            surfaceBlocks_list = [self.surface_meshBlocks[1:-1], self.fault_meshBlocks[1:-1]]

            self.all_surfaceBlocks = f"'{' '.join(surfaceBlocks_list)}'"
            self.all_meshBlocks = f"'{' '.join(meshBlocks_list)}'"
            self.all_faultsVolumesBlocks = f"'{' '.join(volAndFaults_list)}'"
...
#+++++++++++++++++++++++ THIS IS THE START OF THE SIMULATION TEMPLATES +++++++++++++++++++++++++++
    def multiapp_fault_stability(self):
        """
        {self.headerBlock}
        {self.globalBlock}
        {self.meshBlock}
        ...
        ###### preStress sub app simulation #######
        suffix='_preStress'
        self.general_headers_Blk()
        self.general_globals_Blk() #global parameters
        self.global_variables_Blk() #k ratios
        self.general_variables_Blk()  #disp xyz
        
        if self.meshBlockIsBuilt_flag:
            pass
        else:
            self.generalized_no_surfaces_mesh_Blk()
        self.preStressFaultStab_AuxVars()
        self.preStress_physics_Blk()
        self.no_Xfer_Blk()
        self.no_multiApp_Blk()
        self.preStress_Kernels_Blk() #grav
        self.no_atmos_geomech_BC_Blk()
        self.preStress_AuxKernels_Blk()
        self.preStress_materials_Blk(density_dict=self.density_dict)
        ...
        self.global_variables_all_mesh_blocks_Blk() #all_blocks
        
        if self.meshBlockIsBuilt_flag:
            pass
        else:
            self.generalized_no_surfaces_mesh_Blk()
        self.multiAppStressCopyXfer_Blk(sub_filename=sub_filename)
        self.Xfer_Blk()
        self.postStress_AuxVars_Blk()
        self.postStress_FaultStab_physics_Blk()
        self.preStress_Kernels_Blk()
        self.atmos_geomech_BC_Blk()
        self.postStress_FaultStab_AuxKernels_Blk()
        self.no_functions_Blk()
        self.no_ic_Blk()
        self.postStress_FaultStab_materials_Blk(density_dict=self.density_dict)
        ...
        print(f"Exported nodal sourced projected stress fault stability to : {self.write_path}")
    ...
    def fault_stability_projected_stress_sim_with_direction(self):
        ...
        if self.meshBlockIsBuilt_flag:
            ...
            self.generalized_mesher_mesh_Blk()
        self.general_variables_Blk()
        self.postGravity_fault_stab_vars()
        self.solidMech_quasiStatic_physics_Blk()
        self.postGravity_Kernels_Blk()
        self.postGravity_fault_stab_AuxKernels_Blk()
        self.atmos_geomech_BC_Blk()
        self.no_functions_Blk()
        self.ic_initStress_Blk() #self.no_ic_Blk()
        self.postGravity_Projected_stress_materials_Blk()
...
#__________________________________________________________ GLOBAL VARIABLES BLOCK _________________________________________________________________       
    def global_variables_all_mesh_blocks_Blk(self):
        self.globalVariablesBlock = f"""
        all_blocks = {self.all_faultsVolumesBlocks}
        """
    
    def global_variables_Blk(self):
        K_XX, K_XY, K_XZ, K_YX, K_YY, K_YZ, K_ZX, K_ZY, K_ZZ = self.stress_tensor.ravel().round(6)
        self.globalVariablesBlock = f"""
        k_xx = {K_XX}
        k_xy = {K_XY}
        k_xz = {K_XZ}
...
#__________________________________________________________ MULTIAPP BLOCK _________________________________________________________________     

    def no_multiApp_Blk(self):
    ...
    def multiAppStressCopyXfer_Blk(self, sub_filename="./sub.i"):
        ...
        [MultiApps]
            ...
            input_files = {sub_filename}
            execute_on = initial
        []
    []
    """

#__________________________________________________________ TRANSFER BLOCK _________________________________________________________________  
    def no_Xfer_Blk(self):
    ...
    def Xfer_Blk(self):
        ...
        [from_subXX]
            ...
            source_variable = initStressX_X
            variable = initStressXX
            from_multi_app = pre_Stress
        []
        [from_subXY]
            type = MultiAppCopyTransfer
            source_variable = initStressX_Y
            variable = initStressXY
            from_multi_app = pre_Stress
        []
        [from_subXZ]
            type = MultiAppCopyTransfer
            source_variable = initStressX_Z
            variable = initStressXZ
            from_multi_app = pre_Stress
        []
        [from_subYX]
            type = MultiAppCopyTransfer
            source_variable = initStressY_X
            variable = initStressYX
            from_multi_app = pre_Stress
        []
        [from_subYY]
            type = MultiAppCopyTransfer
            source_variable = initStressY_Y
            variable = initStressYY
            from_multi_app = pre_Stress
        []
        [from_subYZ]
            type = MultiAppCopyTransfer
            source_variable = initStressY_Z
            variable = initStressYZ
            from_multi_app = pre_Stress
        []
        [from_subZX]
            type = MultiAppCopyTransfer
            source_variable = initStressZ_X
            variable = initStressZX
            from_multi_app = pre_Stress
        []
        [from_subZY]
            type = MultiAppCopyTransfer
            source_variable = initStressZ_Y
            variable = initStressZY
            from_multi_app = pre_Stress
        []
        [from_subZZ]
            type = MultiAppCopyTransfer
            source_variable = initStressZ_Z
...
#__________________________________________________________ MESH BLOCK _________________________________________________________________
    ...
    def generalized_no_surfaces_mesh_Blk(self):
...
#__________________________________________________________ VARIABLES BLOCK _________________________________________________________________
    def general_variables_Blk(self):
        self.varsBlock = f"""
            [Variables]
                [disp_x]
                []
...
#__________________________________________________________ AuxVARIABLES BLOCK _________________________________________________________________
    ...
    def preStressFaultStab_AuxVars(self):
        self.auxVarsBlock = f"""
           [AuxVariables]
    ...
    def postStress_AuxVars_Blk(self):
        self.auxVarsBlock = f"""
         [AuxVariables]
        ...
        # Element Normal vector components from porous flow module
          [normal_x]
            order = CONSTANT
            family = MONOMIAL
          []
        ...
        # Normal stress vector components
          [normal_traction_vec_x]
            order = CONSTANT
            family = MONOMIAL
          []
          [normal_traction_vec_y]
            order = CONSTANT
            family = MONOMIAL
          []
          [normal_traction_vec_z]
            order = CONSTANT
            family = MONOMIAL
          []
        ...
        # Stress tensor components Nodal       
        ...
        #Shear vector vars many of these should be vectors
        ...
        #Compass azimuth strike
          [strike_azimuth_deg]
            order =CONSTANT
            family = MONOMIAL
        []

        # Rake of max shear stress vector
          [rake_x]
            order = CONSTANT
            family = MONOMIAL
          []
        ...
        #Rake of max shear stress from strike
          [maxShear_angle_rad]
            order = CONSTANT
            family = MONOMIAL
          [] 
        ...
        #Magnitude shear stress on surface
        [shear_stress_surface]
            order = CONSTANT
            family = MONOMIAL
        []
        ...
        """
    def postGravity_fault_stab_vars(self):
        self.auxVarsBlock = f"""
        [AuxVariables]
      #  [normal_vec_magnitude] # Unit vector check
      #      order = CONSTANT
      #      family = MONOMIAL
      #  []
        [eff_normal_stress_magnitude]
        ...
        [] # End AuxVariables Block =====================================================================================================
        """
    def postGravity_AuxVars_Blk(self): ##### DELETE ME --- THIS PAIRS WITH postGravity_Aux_Kernel_Blk THAT DOES NOT EXECUTE NORMAL AND SHEAR STRESS CORRECTLY #####
        self.auxVarsBlock = f"""
        [AuxVariables]
...
#__________________________________________________________ KERNELS BLOCK _________________________________________________________________
    def preGravity_Kernels_Blk(self):
        ...
        self.kernelsBlock = f"""
            ...
            [] # End Kernels Block ===============================================================================================================
        """
    def preStress_Kernels_Blk(self):
        self.kernelsBlock = f"""
        [Kernels]
        [./gravity]
          type = Gravity
...
#__________________________________________________________ AuxKERNELS BLOCK _________________________________________________________________
    ...
    def preStress_AuxKernels_Blk(self):
    ...
    def postStress_FaultStab_AuxKernels_Blk(self):
        self.auxKernelsBlock = f"""
         [AuxKernels]
    # Element specific Normals for each component vector
        [ElemNormal_x]
      ...
      ### ------------     STRIKE / DIP / RAKE of max shear stress  --------------          
      [./strike_x]
          type = ParsedAux
          variable = strike_x
          coupled_variables = 'normal_x normal_y'
          expression = '-normal_y / sqrt(normal_x^2 + normal_y^2)'
        ...
        [./strike_y]
          ...
          variable = strike_y
          coupled_variables = 'normal_x normal_y'
          expression = 'normal_x / sqrt(normal_x^2 + normal_y^2)'
          block = {self.fault_meshBlocks}
        [../]
        [./strike_z]
          type = ParsedAux
        ...
        [../]
         ...
         # Strike azimuth (in degrees, from north, clockwise)
        [./strike_azimuth_deg]
            type = ParsedAux
    ...
    # Rake of maximum shear stress vector, z component
      [compute_rake_z]
        type = ParsedAux
        variable = rake_z
      ...
      []
            ...
            #Shear Stress vector calculations ---

          #Angle Max shear stress in radians from strike
          [./maxShear_stress_rake_rad]
              type = ParsedAux
              variable = maxShear_angle_rad
              execute_on = TIMESTEP_END
          ...
          #Angel of maximum shear stress in degrees from strike
            [maxShear_stress_rake_deg]
              type = ParsedAux
              variable = maxShear_angle_deg
              coupled_variables = 'maxShear_angle_rad'
              #expression = 'atan((maxprincipal - minprincipal)/(minprincipal - maxprincipal))'
              expression = 'maxShear_angle_rad * (180.0/3.141592653589793238463)' # radians to degrees
              block = {self.fault_meshBlocks}
            []   
        ...
        [./NormalStressVec_x]
                type = ParsedAux
                variable = normal_traction_vec_x
                coupled_variables = 'stress_xx stress_xy stress_xz normal_x normal_y normal_z'
                expression = '(normal_x*(stress_xx)) + (normal_y*(stress_xy)) + (normal_z*(stress_xz))'
                block = {self.fault_meshBlocks}
            [../]
        [./NormalStressVec_y]
                type = ParsedAux
                variable = normal_traction_vec_y
                coupled_variables = 'stress_yx stress_yy stress_yz normal_x normal_y normal_z'
                expression = '(normal_x*(stress_yx)) + (normal_y*(stress_yy)) + (normal_z*(stress_yz))'
                block = {self.fault_meshBlocks}
            [../]
        [./NormalStressVec_z]
                type = ParsedAux
                variable = normal_traction_vec_z
                coupled_variables = 'stress_zx stress_zy stress_zz normal_x normal_y normal_z'
                expression = '(normal_x*(stress_zx)) + (normal_y*(stress_zy)) + (normal_z*(stress_zz))'
                block = {self.fault_meshBlocks}
            [../]
        ...
        [./TauD] #Scalar dot product of the inverted dip vector (so it points downward) and the normal stress unit vector (NormalForceVec x,y,z)
                type = ParsedAux
                variable = tau_d
                coupled_variables = 'normal_traction_vec_x normal_traction_vec_y normal_traction_vec_z dip_x dip_y dip_z'
                expression ='(normal_traction_vec_x*(dip_x))+(normal_traction_vec_y*(dip_y))+(normal_traction_vec_z*(dip_z))'
                block = {self.fault_meshBlocks}
        [../]
        ...
        [./TauS] #Scalar dot product of the strike vector and the normal stress unit vector (NormalForceVec x,y,z)
                type = ParsedAux
                variable = tau_s
                coupled_variables = 'normal_traction_vec_x normal_traction_vec_y normal_traction_vec_z strike_x strike_y strike_z'
                expression ='(normal_traction_vec_x*(strike_x))+(normal_traction_vec_y*(strike_y))+(normal_traction_vec_z*(strike_z))'
                block = {self.fault_meshBlocks}
        [../]
        ...
        [./effectiveNormalStress]
            ...
            expression = 'normal_x*(normal_x*(stress_xx) + normal_y*(stress_xy) + normal_z*(stress_xz)) +
                          normal_y*(normal_x*(stress_yx) + normal_y*(stress_yy) + normal_z*(stress_yz)) +
                          normal_z*(normal_x*(stress_zx) + normal_y*(stress_zy) + normal_z*(stress_zz))'
            coupled_variables = 'normal_x normal_y normal_z stress_xx stress_yy stress_zz stress_xy stress_xz stress_yz stress_zx stress_yx stress_zy'
       ...
       [shear_stress_surface]
            type = ParsedAux
            variable = shear_stress_surface
            coupled_variables = 'stress_xx stress_yy stress_zz stress_xy stress_xz stress_yz normal_x normal_y normal_z'
            expression = '
            sqrt(
               pow((abs(stress_xx) * normal_x) + (abs(stress_xy) * normal_y) + (abs(stress_xz) * normal_z), 2) +
               pow((abs(stress_xy) * normal_x) + (abs(stress_yy) * normal_y) + (abs(stress_yz) * normal_z), 2) +
               pow((abs(stress_xz) * normal_x) + (abs(stress_yz) * normal_y) + (abs(stress_zz) * normal_z), 2)
               -
                pow(
                ((abs(stress_xx) * normal_x) + (abs(stress_xy) * normal_y) + (abs(stress_xz) * normal_z)) * normal_x +
                ((abs(stress_xy) * normal_x) + (abs(stress_yy) * normal_y) + (abs(stress_yz) * normal_z)) * normal_y +
                ((abs(stress_xz) * normal_x) + (abs(stress_yz) * normal_y) + (abs(stress_zz) * normal_z)) * normal_z, 2
               )
            )
            '
            block = {self.fault_meshBlocks}
    ...
    # Geomechanical calculations on fault surfaces
      [SlipTendency]
        ...
        coupled_variables = 'shear_stress_surface eff_normal_stress_magnitude'# hydrostatic_FW'
        expression = 'abs(shear_stress_surface)/abs(eff_normal_stress_magnitude)'#'-abs(hydrostatic_FW))'  # must invert the Maxshear stress to a compressional stress (negative in Moose) beacuase MaxShear outputs absolute value
        #execute_on = timestep_end
    ...
    [DilationTendency]
        ...
        expression = '(abs(min_principal_stress)-abs(eff_normal_stress_magnitude))/(abs(min_principal_stress)-abs(max_principal_stress))' # minprinciple = Sigma1 ; maxprinciple = Sigma3
        coupled_variables = 'max_principal_stress min_principal_stress eff_normal_stress_magnitude'
        #execute_on = timestep_end
        check_boundary_restricted = false
        use_xyzt = true
        block = ${{all_blocks}}   
    ...
    [Fracture_Suscpetibility]
        ...
        expression = 'abs(eff_normal_stress_magnitude) - (shear_stress_surface / 0.6)'
    ...
    def postGravity_fault_stab_AuxKernels_Blk(self):
        self.auxKernelsBlock = f"""
        [AuxKernels]
    # Element specific Normals for each component vector
        [ElemNormal_x]
      ...
      #  [strike_vec]  # This Doesn't compute expression_z reliably 
      #    type = ParsedVectorAux
      #    variable = strike_vec # vectorized normal vector
      #    expression_x = '-1*(-normal_y / sqrt(normal_x^2 + normal_y^2))'
      #    expression_y = '-1*(normal_x / sqrt(normal_x^2 + normal_y^2))'
      #    expression_z = '0'
      #    coupled_variables = 'normal_x normal_y normal_z'
      #    #coupled_vector_variables = 'normal_x normal_y normal_z'
      #    #use_xyzt = true
      #    block = {self.all_surfaceBlocks}
      #  []
                          ...
                          #[dip_vec]  # This Doesn't compute expression_z reliably 
                          #        type = ParsedVectorAux
                          #        variable = dip_vec # vectorized dip vector
                          #        expression_x = '-1*((-normal_z * normal_x) / sqrt(normal_x^2 + normal_y^2 + normal_z^2))'
                          #        expression_y = '-1*((normal_z * normal_y) / sqrt(normal_x^2 + normal_y^2 + normal_z^2))'
                          #        expression_z = '-1*((normal_x^2 + normal_y^2) / sqrt(normal_x^2 + normal_y^2 + normal_z^2))'
                          #        coupled_variables = 'normal_x normal_y normal_z'
                          #        #coupled_vector_variables = 'strike_vec normal_vec'
                          #        #use_xyzt = true
                          #        block = {self.all_surfaceBlocks}
                          #  []
    ...
    # Unit vector check, This should always compute to 1
    ...
    #        [../]
        ...
        [./effectiveNormalStress]
            type = ParsedAux
            variable = eff_normal_stress_magnitude
            block = {self.all_surfaceBlocks} # Only compute this on 2D surfaces (Lower Dimensional Blocks)
            # Calculate the stress magnitude along the normal direction
            expression = 'normal_x*(normal_x*(stress_xx) + normal_y*(stress_xy) + normal_z*(stress_xz)) +
                          normal_y*(normal_x*(stress_yx) + normal_y*(stress_yy) + normal_z*(stress_yz)) +
                          normal_z*(normal_x*(stress_zx) + normal_y*(stress_zy) + normal_z*(stress_zz))'#'+hydrostatic_FW'
            coupled_variables = 'normal_x normal_y normal_z stress_xx stress_yy stress_zz stress_xy stress_xz stress_yz stress_zx stress_yx stress_zy' #hydrostatic_FW
    ...
    [DilationTendency]
        type = ParsedAux
        variable = dilation_tendency
        expression = '(minprincipal-eff_normal_stress_magnitude)/(minprincipal-maxprincipal)' # minprinciple = Sigma1 ; maxprinciple = Sigma3
        coupled_variables = 'maxprincipal minprincipal eff_normal_stress_magnitude'
        #execute_on = timestep_end
        check_boundary_restricted = false
        use_xyzt = true
        block = {self.all_surfaceBlocks}   
    ...
    [Fracture_Suscpetibility]
        type = ParsedAux
        variable = fracture_suscept
        expression = 'abs(eff_normal_stress_magnitude) - (shear_stress_surface / 0.85)'
        coupled_variables = 'shear_stress_surface eff_normal_stress_magnitude'
        check_boundary_restricted = false
        use_xyzt = true
        block = {self.all_surfaceBlocks}
    ...
    def postGravity_AuxKernels_Blk(self):  ##### THIS DOES NOT EXECUTE NORMAL AND SHEAR STRESS CORRECTLY #####
...
#__________________________________________________________ PHYSICS BLOCK _________________________________________________________________
    def preStress_physics_Blk(self):
        self.physicsBlock = f"""
            [Physics]
            [SolidMechanics]
                [QuasiStatic]
    ...
    def postStress_FaultStab_physics_Blk(self):
        self.physicsBlock = f"""
            [Physics]
            [SolidMechanics]
                [QuasiStatic]
                [All]
                    ...
                    displacements = 'disp_x disp_y disp_z'
                    eigenstrain_names = ini_stress
                    generate_output = 'max_principal_stress mid_principal_stress min_principal_stress stress_xx stress_xy stress_xz stress_yx stress_yy stress_yz stress_zx stress_zy stress_zz'
                []
                []
            []
            []
...
#__________________________________________________________ BOUNDARY CONDITIONS BLOCK _________________________________________________________________
    ...
    def no_atmos_geomech_BC_Blk(self):
...
#__________________________________________________________ MATERIALS BLOCK ____________________________________________________________
    ...
    def preStress_materials_Blk(self, density_dict={}):
    ...
    def postStress_FaultStab_materials_Blk(self, density_dict={}):
